...
if ( developer() > 0 )
{
	...
	function PrintHelp( string = "*", exact = false )
	{
		printl( "==============" )
		if ( IsServer() )
			printl( "Server script: \"" + string + "\"" )
		else if ( IsClient() )
			printl( "Client script: \"" + string + "\"" )
		else if ( IsUI )
			printl( "UI script: \"" + string + "\"" )
		else
			Assert( false )
		printl( "" )
...
function VSquirrel_OnRegisteredClass( className )
{
	if ( ScriptExists( "class/" + className.tolower() ) )
		::DoIncludeScript( "class/" + className.tolower(), null )
}
...
class CSimpleCallChainer
{
	constructor( prefixString, scopeForThis = null, exactNameMatch = false )
	{
		prefix = prefixString;
		if ( scopeForThis != null )
			scope = scopeForThis.weakref();
		else
			scope = ::getroottable().weakref();
		chain = [];
...
class Schedule
{
	...
	function AddTask( taskName, param = 0 )
	{
		Tasks.append( { name=taskName, param=param } )
	}

	function AddFailSchedule( schedName )
	{
		Tasks.append( { name=taskName, param=param } )
	}
	...
	function AddInterrupt( condName )
	{
		Interrupts.append( condName )
	}
	
	function GetScheduleString()
	{
		local scheduleString = "\n Schedule"
		scheduleString += "\n    " + scheduleID
