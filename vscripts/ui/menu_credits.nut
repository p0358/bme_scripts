...
TIMESCROLLNAME <- null //10 //even division of SCROLLRANGE ... 20
...
function main()
{
	Globalize( InitCreditsMenu )
	Globalize( PlayCredits )

	RegisterSignal( "PlayingCreditsDone" )
}

function InitCreditsMenu( menu )
{
...
{
	local topElem = GetElem( GetMenu( "CreditsMenu" ), "Credits_Centered_0" )
	local botElem = GetElem( GetMenu( "CreditsMenu" ), "Credits_Centered_19" )
	topElem.ReturnToBasePos()
	botElem.ReturnToBasePos()

	CREDITSTOP = topElem.GetPos()[1]
	local bottom = botElem.GetPos()[1]
	local range = bottom - CREDITSTOP
	TIMESCROLLNAME = floor( range.tofloat() / 60.0 )//60 fps
	SCROLLRANGE = TIMESCROLLNAME * 60

	local creditTimeBaseScroll = ( TIMESCROLLNAME.tofloat() / SCROLLRANGE.tofloat() ).tofloat()
	level.creditScreenResRatio = SCROLLRANGE.tofloat() / 600.0
	...
	DEPARTMENTHEIGHT = floor( 40 * level.creditScreenResRatio )
	NAMEHEIGHT = floor( 25 * level.creditScreenResRatio )

	printt( "DEPARTMENTHEIGHT 40: ", DEPARTMENTHEIGHT, "NAMEHEIGHT 25: ", NAMEHEIGHT, "CREDITSTOP 37: ", CREDITSTOP )
	printt( "SCROLLRANGE 600: ", SCROLLRANGE, "TIMESCROLLNAME 10: ", TIMESCROLLNAME )
}

function PlayCredits( skippable = true )
{
	InitResolution()
	thread ClearCredits( 0 )
	local menu, imageBG

	menu 	= GetMenu( "CreditsMenu" )
	imageBG = GetElem( menu, "BackgroundImage" )
	...
	uiGlobal.playingCredits = true
	thread PlayCreditsMusic()
	...
	if ( uiGlobal.activeMenu == GetMenu( "CreditsMenu" ) )
		CloseTopMenu()
...
function PlayCreditsMusic()
{
	...
	wait 0.5
	EmitUISound( "MainMenu_MusicCredits" )
}

function StopCreditsMusic()
{
	StopUISound( "MainMenu_MusicCredits" )
}
...
function RollCreditsOld()
{
	//============================
	//	Top and Executive
	//============================
	DisplaySingleNameWithTitle( "top", "president", 110 )
	wait 1.5
	DisplaySingleNameWithTitle( "top", "gameDirector", 150 )
	wait 1.5
	local time = DisplaySingleColumnNamesWithTitles( "exec", 240 )
	wait FADENAMEINTIME + time
	ClearCredits()

	//============================
	//	DESIGN
	//============================
	local time = DisplaySingleColumnNamesWithTitles( "design", 90 )
	wait FADENAMEINTIME + time
	ClearCredits()
	...
	//	ENGINEERING
	//============================
	local time = DisplaySingleColumnNamesWithTitles( "engineering", 100 )
	wait FADENAMEINTIME + time
	ClearCredits()
	...
	//	ART
	//============================
	local time = DisplaySingleColumnNamesWithTitles( "art", 70 )
	wait FADENAMEINTIME + time
	ClearCredits()

	local time1 = DisplaySingleColumnNamesWithTitles( "fx", 100 )
	wait 0.5
	local time2 = DisplaySingleColumnCenteredNames( "conceptArt", 250 )
	wait FADENAMEINTIME + time1 + time2 - 0.5
	ClearCredits()



	wait FADENAMEOUTTIME
...
function DisplaySingleNameWithTitle( department, title, ypos )
{
	__DisplaySingleSlot( GetNextNameSlot, department, title, null, null, ypos )
}
...
function __ScrollSingleSlot( GetSlotFunc, department, title, name, xpos, scan = false )
{
	...
	thread HideSlot( slot, FADENAMEOUTTIME, TIMESCROLLNAME - FADENAMEOUTTIME )
	if ( scan )
...
function ScanSlot( slot )
{
	...
	time = 0.1
	scan.FadeOverTime( 190, time, INTERPOLATOR_ACCEL )
	scan.ScaleOverTime( 0.7, 0.6, time, INTERPOLATOR_ACCEL )
	wait time

	scan.ScaleOverTime( 0.5, 0.5, time, INTERPOLATOR_DEACCEL )
	scan.FadeOverTime( 20, time, INTERPOLATOR_ACCEL )
	wait time + 0.1

	scaletime = 0.5
	scan.ScaleOverTime( 1.0, 0.0, scaletime, INTERPOLATOR_ACCEL )
	scan.FadeOverTime( 150, time, INTERPOLATOR_ACCEL )

	wait time
	scan.FadeOverTime( 0, scaletime - time, INTERPOLATOR_ACCEL )
...
function ScrollElement( element, xpos, top = null )
{
	if ( !top )
		top = CREDITSTOP
	local bot = SCROLLRANGE + top

	MoveElement( element, xpos, bot )
	element.MoveOverTime( element.GetPos()[0], top, TIMESCROLLNAME )//INTERPOLATOR_LINEAR
}
...
function __ShowRespawnLogoThread( delay )
{
	...
	logo.ColorOverTime( 255, 255, 255, 255, FADENAMEINTIME, INTERPOLATOR_ACCEL )

	local bot = SCROLLRANGE - ( SCROLLRANGE * 0.5 )
	...
	logo.ColorOverTime( 0, 0, 0, 255, FADENAMEOUTTIME, INTERPOLATOR_ACCEL )
...
function ShowSlot( slot, name, title = null )
{
	...
	slot.name.Show()
	slot.name.SetAlpha( 0 )
	slot.name.FadeOverTime( 255, FADENAMEINTIME, INTERPOLATOR_ACCEL )

	if ( !title )
		return

	slot.title.SetText( title )
	slot.title.Show()
	slot.title.SetAlpha( 0 )
	slot.title.FadeOverTime( 255, FADENAMEINTIME, INTERPOLATOR_ACCEL )
}
...
function HideSlot( slot, fadeout = FADENAMEOUTTIME, delay = null )
{
	...
	slot.name.FadeOverTime( 0, fadeout, INTERPOLATOR_ACCEL )
	if ( "title" in slot )
		slot.title.FadeOverTime( 0, fadeout, INTERPOLATOR_ACCEL )
...
{
	return string + "_ut" + __uniqueTitleId++;
}
