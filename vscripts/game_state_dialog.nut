function main()
{
	...
	if ( GameModeHasCapturePoints() )
		...
		IncludeScript( "_gamemode_ctf_dialogue" )

	// GameModeHasCapturePoints() return true for coop so I couln't just do an additional else if
	if ( GameRules.GetGameMode() == COOPERATIVE )
		IncludeScript( "_gamemode_coop_dialogue" )

	IncludeScript( "_evac_dialogue" )

	local map = GetMapName()
	local testMap = "test/" + map + "_dialog"
	...
	if ( ScriptExists( testMap ) )
	{
		IncludeFile( testMap )
	}

	// need to call this after base dialog has been overridden
	if ( GameModeHasCapturePoints() )
		RegisterHardpointConversations()
	else if ( GameRules.GetGameMode() == "ctf" )
		RegisterCTFConversations()

	// GameModeHasCapturePoints() return true for coop so I couln't just do an additional else if
	if ( GameRules.GetGameMode() == COOPERATIVE )
		RegisterCoopConversations()

	switch ( GAMETYPE ) //Older game modes that don't use new style of game mode scripts
	{
...
function RegisterCalloutConversations()
{
	//Register conversations tied to the score system found in _score_shared.nut here.

	RegisterConversation( "TitanStepCrush",		VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "PilotEjectKill",		VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "FirstStrike",		VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "MegaKill",			VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "TripleKill",			VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "DoubleKill",			VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "Showstopper",		VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "Rampage",			VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "KillingSpree",		VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "Dominating",			VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "Nemesis",			VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "Comeback",			VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "Bullseye",			VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "RodeoConnect",		VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "RodeoRake",			VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "KillStryder",		VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "KillAtlas",			VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "KillOgre",			VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "WorthIt",			VO_PRIORITY_PLAYERSTATE )
	...
	/************************ Bullseye ***************************/
	AddVDULineForBlisk( "Bullseye", "diag_imc_blisk_kc_bullseye_01" ) //Blisk: Bullseye!
	AddVDUAnimWithEmbeddedAudioForBlisk( "Bullseye", "diag_imc_blisk_kc_bullseye_02" ) //Blisk: Great shot!!

	/************************ Rodeo Connect ***************************/
	AddVDULineForBlisk( "RodeoConnect", "diag_imc_blisk_kc_rodeo_01" ) //Blisk: Bring it down.
	AddVDULineForBlisk( "RodeoConnect", "diag_imc_blisk_kc_rodeo_02" ) //Blisk: Take it down.
	AddVDULineForBlisk( "RodeoConnect", "diag_imc_blisk_kc_rodeo_03" ) //Blisk: Nice moves, now finish the job.
	...
	AddVDUAnimWithEmbeddedAudioForBlisk( "KillStryder", "diag_imc_blisk_kc_pilotkilltitan_01" ) //Blisk: The bigger they are, the harder they fall.
	AddVDUAnimWithEmbeddedAudioForBlisk( "KillStryder", "diag_imc_blisk_kc_pilotkilltitan_02" ) //Blisk: Timber!
	/************************ Pilot Kill Atlas ***************************/
	AddVDUAnimWithEmbeddedAudioForBlisk( "KillAtlas", "diag_imc_blisk_kc_pilotkillatlas_04" ) //Blisk: Impressive - you just took down an Atlas.
	AddVDUAnimWithEmbeddedAudioForBlisk( "KillAtlas", "diag_imc_blisk_kc_pilotkilltitan_01" ) //Blisk: The bigger they are, the harder they fall.
	AddVDUAnimWithEmbeddedAudioForBlisk( "KillAtlas", "diag_imc_blisk_kc_pilotkilltitan_02" ) //Blisk: Timber!
	/************************ Pilot Kill Ogre ***************************/
	AddVDULineForBlisk( "KillOgre", "diag_imc_blisk_kc_pilotkillogre_05" ) //Blisk: You just downed an Ogre.
	AddVDUAnimWithEmbeddedAudioForBlisk( "KillOgre", "diag_imc_blisk_kc_pilotkilltitan_01" ) //Blisk: The bigger they are, the harder they fall.
	AddVDUAnimWithEmbeddedAudioForBlisk( "KillOgre", "diag_imc_blisk_kc_pilotkilltitan_02" ) //Blisk: Timber!
	/************************ Worth It ***************************/
	AddVDULineForBlisk( "WorthIt", "diag_imc_blisk_kc_worthit_01" ) //Blisk: Worth it.
	AddVDULineForBlisk( "WorthIt", "diag_imc_blisk_kc_worthit_02" ) //Blisk: Going down in a blaze of glory eh?
	AddVDULineForBlisk( "WorthIt", "diag_imc_blisk_kc_worthit_03" ) //Blisk: Nice job taking them down with you.

	/************************ Titan Step Crush ***************************/
	AddVDULineForBlisk( "TitanStepCrush", "diag_imc_blisk_kc_hitandrun_01" ) //Blisk: That must have hurt.
	AddVDULineForBlisk( "TitanStepCrush", "diag_imc_blisk_kc_hitandrun_02" ) //Blisk: You get to wash that off.

	/************************ Pilot Eject Kill ***************************/
	AddVDULineForBlisk( "PilotEjectKill", "diag_imc_blisk_kc_pilotejectkill_01" ) //Blisk: Pull!
	AddVDULineForBlisk( "PilotEjectKill", "diag_imc_blisk_kc_pilotejectkill_02" ) //Blisk: Nice shooting.
	AddVDULineForBlisk( "PilotEjectKill", "diag_imc_blisk_kc_pilotejectkill_03" ) //Blisk: You almost feel bad for them.
	AddVDULineForBlisk( "PilotEjectKill", "diag_imc_blisk_kc_pilotejectkill_04" ) //Blisk: That pilot didn't stand a chance.

	/************************ First Strike ***************************/
	AddVDULineForBlisk( "FirstStrike", "diag_imc_blisk_kc_firstblood_01" ) //Blisk: Not bad.
	AddVDULineForBlisk( "FirstStrike", "diag_imc_blisk_kc_firstblood_02" ) //Blisk: You've got talent, I'll give you that.
	AddVDULineForBlisk( "FirstStrike", "diag_imc_blisk_kc_firstblood_03" ) //Blisk: First blood.

	/************************ Mega Kill ***************************/
	AddVDULineForBlisk( "MegaKill", "diag_imc_blisk_kc_megakill_01" ) //Blisk: Ahh it's like shootin' fish in a barrel!
	AddVDULineForBlisk( "MegaKill", "diag_imc_blisk_kc_megakill_02" ) //Blisk: I lost count.
	AddVDULineForBlisk( "MegaKill", "diag_imc_blisk_kc_megakill_03" ) //Blisk: That was AWESOME.
	AddVDULineForBlisk( "MegaKill", "diag_imc_blisk_kc_megakill_04" ) //Blisk: That's what I call a Mega Kill.

	/************************ Triple Kill ***************************/
	AddVDULineForBlisk( "TripleKill", "diag_imc_blisk_kc_triplekill_01" ) //Blisk: Nice hat trick.
	AddVDULineForBlisk( "TripleKill", "diag_imc_blisk_kc_triplekill_02" ) //Blisk: Three in a row.
	AddVDULineForBlisk( "TripleKill", "diag_imc_blisk_kc_triplekill_03" ) //Blisk: That's a triple kill.

	/************************ Double Kill ***************************/
	AddVDULineForBlisk( "DoubleKill", "diag_imc_blisk_kc_doublekill_01" ) //Blisk: That's two down.
	AddVDULineForBlisk( "DoubleKill", "diag_imc_blisk_kc_doublekill_02" ) //Blisk: That's two in a row.
	AddVDULineForBlisk( "DoubleKill", "diag_imc_blisk_kc_doublekill_03" ) //Blisk: Double the fun.
	AddVDULineForBlisk( "DoubleKill", "diag_imc_blisk_kc_doublekill_04" ) //Blisk: That's a double kill.

	/************************ Showstopper ***************************/
	AddVDULineForBlisk( "Showstopper", "diag_imc_blisk_kc_iced_01" ) //Blisk: That should cool him off.
	AddVDULineForBlisk( "Showstopper", "diag_imc_blisk_kc_iced_02" ) //Blisk: Shut down!

	/************************ Rampage ***************************/
	AddVDULineForBlisk( "Rampage", "diag_imc_blisk_kc_rampage_01" ) //Blisk: You're unstoppable.
	AddVDULineForBlisk( "Rampage", "diag_imc_blisk_kc_rampage_02" ) //Blisk: Rock and rolling.

	/************************ Killing Spree ***************************/
	AddVDULineForBlisk( "KillingSpree", "diag_imc_blisk_kc_killingspree_01" ) //Blisk: Impressive streak.
	AddVDULineForBlisk( "KillingSpree", "diag_imc_blisk_kc_killingspree_02" ) //Blisk: Nice moves.

	/************************ Dominating ***************************/
	AddVDULineForBlisk( "Dominating", "diag_imc_blisk_kc_dominating_01" ) //Blisk: You're picking that pilot apart.
	AddVDULineForBlisk( "Dominating", "diag_imc_blisk_kc_dominating_02" ) //Blisk: You're dominating that pilot.
	AddVDULineForBlisk( "Dominating", "diag_imc_blisk_kc_dominating_03" ) //Blisk: Dominating.
	AddVDULineForBlisk( "Dominating", "diag_imc_blisk_kc_dominating_04" ) //Blisk: He's going to hold a grudge.
	/************************ Nemesis ***************************/
	AddVDULineForBlisk( "Nemesis", "diag_imc_blisk_kc_retribution_01" ) //Blisk: You got him!
	AddVDULineForBlisk( "Nemesis", "diag_imc_blisk_kc_retribution_02" ) //Blisk: That's how you settle the score.
	/************************ Comeback ***************************/
	AddVDULineForBlisk( "Comeback", "diag_imc_blisk_kc_comeback_01" ) //Blisk: Back in the game
	AddVDULineForBlisk( "Comeback", "diag_imc_blisk_kc_comeback_02" ) //Blisk: Fresh start.
	AddVDULineForBlisk( "Comeback", "diag_imc_blisk_kc_comeback_03" ) //Blisk: Clean slate.
	AddVDULineForBlisk( "Comeback", "diag_imc_blisk_kc_comeback_04" ) //Blisk: Way to shake it off.


	/**************************************************************************/
	/************************        MILITIA        ***************************/
	/************************ Bullseye ***************************/
	AddVDULineForBish( "Bullseye", "diag_kc_bish_bullseye_01" ) //Bish: Bullseye!
	AddVDULineForBish( "Bullseye", "diag_kc_bish_bullseye_02" ) //Bish: Great shot!!

	/************************ Rodeo Connect ***************************/
	AddVDULineForBish( "RodeoConnect", "diag_kc_bish_rodeo_01" ) //Bish: Bring it down.
	AddVDULineForBish( "RodeoConnect", "diag_kc_bish_rodeo_02" ) //Bish: Take it down.
	AddVDULineForBish( "RodeoConnect", "diag_kc_bish_rodeo_03" ) //Bish: Nice moves, now finish the job.

	/************************ Rodeo Rake ***************************/
	AddVDULineForBish( "RodeoRake", "diag_kc_bish_rakerodeoguy_01" ) //Bish: Way to watch their back!
	AddVDULineForBish( "RodeoRake", "diag_kc_bish_rakerodeoguy_02" ) //Bish: Nice save!
	AddVDULineForBish( "RodeoRake", "diag_kc_bish_rakerodeoguy_03" ) //Bish: They owe you one!

	/************************ Pilot Kill Stryder ***************************/
	AddVDULineForBish( "KillStryder", "diag_kc_bish_pilotkillstryder_03" ) //Bish: That's a Stryder down.
	AddVDULineForBish( "KillStryder", "diag_kc_bish_pilotkilltitan_01" ) //Bish: The bigger they are, the harder they fall.
	AddVDULineForBish( "KillStryder", "diag_kc_bish_pilotkilltitan_02" ) //Bish: Timber!
	/************************ Pilot Kill Atlas ***************************/
	AddVDULineForBish( "KillAtlas", "diag_kc_bish_pilotkillatlas_04" ) //Bish: Atlas down!
	AddVDULineForBish( "KillAtlas", "diag_kc_bish_pilotkilltitan_01" ) //Bish: The bigger they are, the harder they fall.
	AddVDULineForBish( "KillAtlas", "diag_kc_bish_pilotkilltitan_02" ) //Bish: Timber!
	/************************ Pilot Kill Ogre ***************************/
	AddVDULineForBish( "KillOgre", "diag_kc_bish_pilotkillogre_05" ) //Bish: You just downed an Ogre.
	AddVDULineForBish( "KillOgre", "diag_kc_bish_pilotkilltitan_01" ) //Bish: The bigger they are, the harder they fall.
	AddVDULineForBish( "KillOgre", "diag_kc_bish_pilotkilltitan_02" ) //Bish: Timber!
	/************************ Worth It ***************************/
	AddVDULineForBish( "WorthIt", "diag_kc_bish_worthit_01" ) //Bish: Worth it.
	AddVDULineForBish( "WorthIt", "diag_kc_bish_worthit_02" ) //Bish: Going down in a blaze of glory
	AddVDULineForBish( "WorthIt", "diag_kc_bish_worthit_03" ) //Bish: Nice job taking them down with you.

	/************************ Titan Step Crush ***************************/
	AddVDULineForBish( "TitanStepCrush", "diag_kc_bish_hitandrun_01" ) //Bish: That must have hurt.
	AddVDULineForBish( "TitanStepCrush", "diag_kc_bish_hitandrun_02" ) //Bish: You get to wash that off.

	/************************ Pilot Eject Kill ***************************/
	AddVDULineForBish( "PilotEjectKill", "diag_kc_bish_pilotejectkill_01" ) //Bish: Pull!
	AddVDULineForBish( "PilotEjectKill", "diag_kc_bish_pilotejectkill_02" ) //Bish: Nice shooting.
	AddVDULineForBish( "PilotEjectKill", "diag_kc_bish_pilotejectkill_03" ) //Bish: You almost feel bad for them... Naah
	AddVDULineForBish( "PilotEjectKill", "diag_kc_bish_pilotejectkill_04" ) //Bish: That pilot didn't stand a chance.

	/************************ First Strike ***************************/
	AddVDULineForBish( "FirstStrike", "diag_kc_bish_firstblood_01" ) //Bish: You're off to a good start!
	AddVDULineForBish( "FirstStrike", "diag_kc_bish_firstblood_02" ) //Bish: Way to get it started.
	AddVDULineForBish( "FirstStrike", "diag_kc_bish_firstblood_03" ) //Bish: First blood.

	/************************ Mega Kill ***************************/
	AddVDULineForBish( "MegaKill", "diag_kc_bish_megakill_01" ) //Bish: Like shootin' fish in a barrel!
	AddVDULineForBish( "MegaKill", "diag_kc_bish_megakill_02" ) //Bish: I lost count.
	AddVDULineForBish( "MegaKill", "diag_kc_bish_megakill_03" ) //Bish: That was AWESOME.
	AddVDULineForBish( "MegaKill", "diag_kc_bish_megakill_04" ) //Bish: That's what I call a Mega Kill.

	/************************ Triple Kill ***************************/
	AddVDULineForBish( "TripleKill", "diag_kc_bish_triplekill_01" ) //Bish: Hat trick! Nice.
	AddVDULineForBish( "TripleKill", "diag_kc_bish_triplekill_02" ) //Bish: Three in a row.
	AddVDULineForBish( "TripleKill", "diag_kc_bish_triplekill_03" ) //Bish: That's a triple kill.

	/************************ Double Kill ***************************/
	AddVDULineForBish( "DoubleKill", "diag_kc_bish_doublekill_01" ) //Bish: That's two down.
	AddVDULineForBish( "DoubleKill", "diag_kc_bish_doublekill_02" ) //Bish: Two in a row.
	AddVDULineForBish( "DoubleKill", "diag_kc_bish_doublekill_03" ) //Bish: Double the fun.
	AddVDULineForBish( "DoubleKill", "diag_kc_bish_doublekill_04" ) //Bish: That's a double kill.

	/************************ Showstopper ***************************/
	AddVDULineForBish( "Showstopper", "diag_kc_bish_iced_01" ) //Bish: That should cool him off.
	AddVDULineForBish( "Showstopper", "diag_kc_bish_iced_02" ) //Bish: Shut down!

	/************************ Rampage ***************************/
	AddVDULineForBish( "Rampage", "diag_kc_bish_rampage_01" ) //Bish: You're unstoppable.
	AddVDULineForBish( "Rampage", "diag_kc_bish_rampage_02" ) //Bish: Rock and rolling.

	/************************ Killing Spree ***************************/
	AddVDULineForBish( "KillingSpree", "diag_kc_bish_killingspree_01" ) //Bish: Impressive streak.
	AddVDULineForBish( "KillingSpree", "diag_kc_bish_killingspree_02" ) //Bish: Nice moves.

	/************************ Dominating ***************************/
	AddVDULineForBish( "Dominating", "diag_kc_bish_dominating_01" ) //Bish: You're picking that pilot apart.
	AddVDULineForBish( "Dominating", "diag_kc_bish_dominating_02" ) //Bish: You're dominating that pilot.
	AddVDULineForBish( "Dominating", "diag_kc_bish_dominating_03" ) //Bish: Dominating.
	AddVDULineForBish( "Dominating", "diag_kc_bish_dominating_04" ) //Bish: He's going to hold a grudge.
	/************************ Nemesis ***************************/
	AddVDULineForBish( "Nemesis", "diag_kc_bish_retribution_01" ) //Bish: You got him!
	AddVDULineForBish( "Nemesis", "diag_kc_bish_retribution_02" ) //Bish: Way to settle the score
	/************************ Comeback ***************************/
	AddVDULineForBish( "Comeback", "diag_kc_bish_comeback_01" ) //Bish: Back in the game
	AddVDULineForBish( "Comeback", "diag_kc_bish_comeback_02" ) //Bish: Fresh start.
	AddVDULineForBish( "Comeback", "diag_kc_bish_comeback_03" ) //Bish: Clean slate.
	AddVDULineForBish( "Comeback", "diag_kc_bish_comeback_04" ) //Bish: Way to shake it off.
}

function RegisterAutoTitanConversations()
{

	foreach( _, modifiedAliasSuffix in  TITAN_OS_VOICE_PACK )
	{
		RegisterConversation( "autotitan" + modifiedAliasSuffix + "_engaged_pilot", VO_PRIORITY_PLAYERSTATE )
		RegisterConversation( "autotitan" + modifiedAliasSuffix + "_engaged_grunt", VO_PRIORITY_PLAYERSTATE )
		RegisterConversation( "autotitan" + modifiedAliasSuffix + "_engaged_titan", VO_PRIORITY_PLAYERSTATE )
		RegisterConversation( "autotitan" + modifiedAliasSuffix + "_engaged_titans", VO_PRIORITY_PLAYERSTATE )
		...
		local lines = [	{ dialogType = "radio",
			choices = [
				"diag_gs_titan" + modifiedAliasSuffix + "_def_autoengagetitans_01"
				"diag_gs_titan" + modifiedAliasSuffix + "_def_autoengagetitans_02"
			]}	]

		AddConversation( "autotitan" + modifiedAliasSuffix + "_engaged_titans", TEAM_MILITIA, lines )
		AddConversation( "autotitan" + modifiedAliasSuffix + "_engaged_titans", TEAM_IMC, lines )

		local lines = [	{ dialogType = "radio",
			choices = [
				"diag_gs_titan" + modifiedAliasSuffix + "_def_autoengagetitan_01"
				"diag_gs_titan" + modifiedAliasSuffix + "_def_autoengagetitan_02"
			]}	]

		AddConversation( "autotitan" + modifiedAliasSuffix + "_engaged_titan", TEAM_MILITIA, lines )
		AddConversation( "autotitan" + modifiedAliasSuffix + "_engaged_titan", TEAM_IMC, lines )

		local lines = [	{ dialogType = "radio",
			choices = [
				"diag_gs_titan" + modifiedAliasSuffix + "_def_autoengagegrunt_01"
				"diag_gs_titan" + modifiedAliasSuffix + "_def_autoengagegrunt_02"
			]}	]

		AddConversation( "autotitan" + modifiedAliasSuffix + "_engaged_grunt", TEAM_MILITIA, lines )
		AddConversation( "autotitan" + modifiedAliasSuffix + "_engaged_grunt", TEAM_IMC, lines )

		local lines = [	{ dialogType = "radio",
			choices = [
				"diag_gs_titan" + modifiedAliasSuffix + "_def_autoengagepilot_01"
				"diag_gs_titan" + modifiedAliasSuffix + "_def_autoengagepilot_02"
			]}	]

		AddConversation( "autotitan" + modifiedAliasSuffix + "_engaged_pilot", TEAM_MILITIA, lines )
		AddConversation( "autotitan" + modifiedAliasSuffix + "_engaged_pilot", TEAM_IMC, lines )
    }
}

//dev only, DO NOT SHIP
function RegisterDebugVDUConversations()
{
	RegisterConversation( "SarahVDU",							VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "MacVDU",								VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "BishVDU",							VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "SpyglassVDU",						VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "GravesVDU",							VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "BliskVDU",							VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "MCORGruntBattleRifleVDU",			VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "MCORGruntAntiTitanVDU",					VO_PRIORITY_PLAYERSTATE )
	RegisterConversation( "IMCSoldierBattleRifleVDU",				VO_PRIORITY_PLAYERSTATE )

	if ( IsServer() )
		return

	//Debug, so registering lines for both sides with the same characters to prevent having to switch teams and other shenningans
	...
	local convRef = AddConversation( "SarahVDU", TEAM_IMC )
	AddVDURadio( convRef, "sarah", null, "diag_mcor_sarah_firsttitan_01" )
