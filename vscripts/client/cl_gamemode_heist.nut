function main()
{
	...
	AddCallback_OnClientScriptInit( HEIST_AddPlayer )
}

function HEIST_AddPlayer( player )
{
...
function SCB_BBUpdate( playerHandle )
{
	...
	if ( player == panelUser )
	{
		...
		msg = "#HEIST_YOU_HACKED_PANEL"
		subMsg = "#HEIST_GET_TO_EVAC"
	}
	else
	{
		if ( team == 0 )
		{
			msg = "#HEIST_DATA_STOLEN"
		}
		else if ( team == level.nv.attackingTeam )
		{
			msg = "#HEIST_TEAM_HACKED_PANEL"
			subMsg = "#HEIST_GET_TO_EVAC"
		}
		else
		{
			msg = "#HEIST_DATA_STOLEN"
			subMsg = "#HEIST_STOP_THE_EVAC"
		}
	}
